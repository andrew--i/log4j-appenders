...
public class BucketWriter {
	...
	public BucketWriter(long rollInterval, long rollSize, long rollCount, long batchSize,
											String filePath, String fileName, String inUsePrefix,
											String inUseSuffix, String fileSuffix, CompressionCodec codeC,
											CompressionType compType, HDFSWriter writer,
											ScheduledExecutorService timedRollerPool, int idleTimeout,
											long callTimeout,
	...
	private Method getRefIsClosed() {
		try {
			...
			LOG.info("isFileClosed() is not available in the version of the " +
					...
					"Writer will not attempt to re-close files if the close fails " +
	 ...
	 * @throws InterruptedException
	 */
	public synchronized void append(final LoggingEvent event, final Layout layout)
			throws IOException, InterruptedException {
		checkAndThrowInterruptedException();
		...
		try {
			callWithTimeout(new CallRunner<Void>() {
				@Override
				public Void call() throws Exception {
					writer.append(event, layout); // could block
					return null;
				}
			});
		} catch (IOException e) {
		...
		// update statistics
		processSize += layout.format(event).length();
		eventCounter++;
		batchCounter++;
